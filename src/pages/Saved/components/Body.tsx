...
const Body: FC = () => {
  ...
  const getWeather = async (e: any, { value }: any) => {
    ...
    const res = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${value}&units=metric&appid=${process.env.REACT_APP_API_KEY}`
    );
    ...
    if (!res.ok) {
      ...
      if (data.main.feels_like < -10) {
        ...
        tempImg =
          "https://c4.wallpaperflare.com/wallpaper/364/79/684/snow-8k-tree-forest-wallpaper-preview.jpg";
      } else if (data.main.feels_like >= -10 && data.main.feels_like <= 5) {
        tempComment = "It's pretty cold out there, don't forget your jacket!";
        tempImg =
          "https://c4.wallpaperflare.com/wallpaper/350/698/382/nature-landscapes-trees-forests-autumn-fall-seasons-winter-snow-frost-shore-lakes-grass-leaves-cold-high-resolution-images-wallpaper-preview.jpg";
      } else if (data.main.feels_like > 5 && data.main.feels_like < 20) {
        tempComment = "It's slightly chilly out there!";
        tempImg =
          "https://hdfreewallpaper.net/wp-content/uploads/2015/08/nice-summer-season-hd-wallpapers-free.jpg";
      } else if (data.main.feels_like >= 20 && data.main.feels_like <= 30) {
        tempComment = "Enjoy the great weather!";
        tempImg =
          "https://p4.wallpaperbetter.com/wallpaper/1021/17/157/summer-scenery-wallpaper-preview.jpg";
      } else {
        tempComment = "It's pretty hot, don't forget your sunglasses!";
        tempImg =
          "https://www.teahub.io/photos/full/288-2884225_temperature-hot-weather.jpg";
      }
      setCitiesData(data);
    }
  };
          ...
          <div className="fav-wrapper">
            <div className="fav-weather-data" data-aos="flip-down">
              <Card fluid>
                ...
                <Card.Content>
                  <Card.Header
                    className="fav-city-title"
                    data-aos="fade-down-right"
                  >
                    <Icon name="map marker alternate" size="small" />
                    {citiesData.name} {citiesData.sys.country}
                  </Card.Header>
                  <Card.Meta>
                    <span className="date" data-aos="fade-up-left">
                      {tempComment}
                    </span>
                  </Card.Meta>
                  <Card.Description>
                    <p data-aos="fade-up-right">
                      <Icon name="circle outline" size="tiny" />
                      Current Temp: {citiesData.main.temp}
                      <sup>&#8451;</sup>
                    </p>
                    <p data-aos="fade-down-left">
                      <Icon name="sun" size="small" />
                      Max Temp: {citiesData.main.temp_max}
                      <sup>&#8451;</sup>
                    </p>
                    <p data-aos="fade-up-right">
                      <Icon name="snowflake" size="small" />
                      Min Temp: {citiesData.main.temp_min}
                      <sup>&#8451;</sup>
                    </p>
                    <p data-aos="fade-down-left">
                      <Icon name="circle outline" size="tiny" />
                      Feels Like: {citiesData.main.feels_like}
                      <sup>&#8451;</sup>
                    </p>
                    <p data-aos="fade-down-right">
                      <Icon name="rain" size="small" />
                      Humidity: {citiesData.main.humidity}%
                    </p>
                    <p data-aos="fade-up-left">
                      <Icon name="leaf" size="small" />
                      Wind Speed: {citiesData.wind.speed}m/s
                    </p>
                  </Card.Description>
                </Card.Content>
                <Card.Content extra>
              ...
              <small data-aos="zoom-in">
                <a
                  href={`https://www.accuweather.com/en/search-locations?query=${citiesData.name}`}
                  target="_blank"
                  rel="noreferrer"
                >
